...
class Commands:
  ...
  async def infodm(self, ctx, *, reason=""):
    if "458341993328803850" in(role.id for role in ctx.message.author.roles):
      ...
      embed.add_field(name='**Random Choice**', value= '{}'.format(random.choice(ctx.message.server.members).mention), inline=True)
      embed.add_field(name='**Random Choice 2**', value= '{}'.format(random.choice(ctx.message.server.members).mention), inline=True)
